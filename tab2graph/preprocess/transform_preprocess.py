...
class RDBTransformPreprocess(RDBDatasetPreprocess):
    ...
    def __init__(self, config : RDBTransformPreprocessConfig, dataset_name=None, device=None):
        super().__init__(config)
        self.transforms = []
        for xcfg in config.transforms:
            xform_class = get_rdb_transform_class(xcfg.name)
            if xcfg.name == "griffin_feature" or xcfg.name == "griffin_feature_split_column":
                xcfg.config['dataset_name'] = dataset_name
                xcfg.config['device'] = device
    ...
    def run(
        ...
        self.output_tasks(ctor, new_task_data_fit, new_task_data_transform, dataset, new_rdb_data)
        ...
        for task_id, task in enumerate(dataset.tasks):
            ...
            for col_schema in task.metadata.columns:
                ...
                if col == task.metadata.time_column:
                    col_meta['is_time_column'] = True
                if col == task.metadata.target_column:
                    col_meta['is_target_column'] = True
                else:
                    col_meta['is_target_column'] = False
    ...
    def output_tasks(
        ...
        for orig_task in ds.tasks:
            ...
            if 'name_emb' in list(task_data_fit[task_name].tables.values())[0][orig_task.metadata.target_column].metadata:
                global task_emb_generator
                if task_emb_generator is None:
                    task_emb_generator = GriffinTaskEmbedding()
                task_emb = task_emb_generator.encode(ctor.name, task_name)
            ...
            if orig_task.metadata.task_type == DBBTaskType.classification:
                if orig_task.metadata.target_column in all_data:
                    label_col = all_data[orig_task.metadata.target_column].data
                    num_classes = len(pd.unique(label_col))
                else:
                    logger.info(f"Using original task {task_name} to infer num_classes.")
                    num_classes = orig_task.metadata.num_classes
...
class GriffinTaskEmbedding:
    def __init__(self):
        self.read_template("configs/transform/task_embedding_template.yaml")
        self.LLM_name = "nomic"
        ...
        if self.LLM_name == "nomic":
            ...
            self.encode = self._nomic_encode
    ...
    def _nomic_encode(self, dataset_name, task_name):
        text = self.template[dataset_name][task_name]['description']
        print(text)
    ...
    def encode(self, dataset_name, task_name):
