...
class DBBNegativeSampler(MiniBatchTransformer):
    def __init__(
        ...
        strict_mode : StrictType = StrictType.random,
        ...
        if self.num_classes is None:
            ...
            self.negative_ratio = (
                self.num_classes - 1 if strict_mode == StrictType.full else negative_ratio
            )
    ...
    def _sample_one_type(
        self, node_pairs: Tuple[torch.Tensor, torch.Tensor], etype: str
        ...
        minibatch = negative_sampling(
            ...
            with_num_classes=self.num_classes is not None,
        )
...
def negative_sampling(
    ...
    shuffle_rest_columns=False,
    strict_mode=StrictType.random,
    with_num_classes=False,
    ...
    # Strict_mode means that we do not allow negative samples to be the same as positive samples.
    # Note the strict_mode only allows one positive link between src & dst.
    if strict_mode == StrictType.full and with_num_classes:
        # Used for Griffin classfication task
        neg_samples = torch.repeat_interleave(
            torch.arange(0, negative_sampling_ratio), size
        )
        # pos_samples = target_column.repeat(negative_sampling_ratio)
        neg_samples = torch.where(
            neg_samples == target_column.repeat(negative_sampling_ratio),
            negative_sampling_ratio,
            neg_samples,
        )
        # del pos_samples
    elif strict_mode == StrictType.full and not with_num_classes:
        # Used for Griffin link prediction task
        neg_samples = torch.randint(
            0, target_column_capacity, (size * negative_sampling_ratio,)
        )
    elif strict_mode == StrictType.random_no_positive:
        # TODO: Check in target_column_capacity != num_classes
        neg_samples = torch.randint(
            0, target_column_capacity - 1, (size * negative_sampling_ratio,)
        )
        pos_samples = target_column.repeat(negative_sampling_ratio)
        neg_samples = torch.where(
            neg_samples == pos_samples, target_column_capacity - 1, neg_samples
        )
        del pos_samples
    elif strict_mode == StrictType.random:
        neg_samples = torch.randint(
            0, target_column_capacity, (size * negative_sampling_ratio,)
        )
    else:
    ...
    for key in list(minibatch.keys()):
        ...
        if shuffle_rest_columns:
            val_corrupted = [
                val[torch.randperm(size)] for _ in range(negative_sampling_ratio)
            ]
    ...
    key_prediction_label = torch.zeros(
        size * (negative_sampling_ratio + 1), dtype=torch.int64
    )
    key_prediction_label[:size] = 1

    minibatch[key_prediction_label_column] = key_prediction_label
    minibatch[key_prediction_query_idx_column] = torch.arange(size).repeat(
