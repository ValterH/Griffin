...
class GraphDatasetMultiTaskConfig(pydantic.BaseModel):
    graph : GraphConfig
    node_features : Dict[NType, Dict[str, FeatureConfig]]
    edge_features : Dict[EType, Dict[str, FeatureConfig]]
    seed_features_dict : Dict[str, Dict[str, FeatureConfig]]
    task_dict : Dict[str, TaskConfig]
    feature_groups : Optional[List[List[DBBGraphFeatureID]]]
...
def parse_config_from_graph_dataset_multitask(
    dataset : DBBGraphDataset,
    ...
    for etype in etypes:
        ...
        if etuple[1].startswith('reverse_'):
            ...
            rev_type = 'reverse_' + etuple[1]
        reverse_etypes_mapping[etype] = f"{etuple[2]}:{rev_type}:{etuple[0]}"
        reverse_etypes_mapping[f"{etuple[2]}:{rev_type}:{etuple[0]}"] = etype
    g_cfg = GraphConfig(
        num_nodes=dataset.graph.num_nodes,
        num_edges=dataset.graph.num_edges,
        ntypes=ntypes,
    ...
    task_cfg_dict = {}
    sfeat_task_cfg = {}
    for task_name in sorted(task_name_list):
        task_cfg = dataset.graph_tasks[task_name].metadata
        task_cfg_dict[task_name] = task_cfg
        ...
        sfeat_task_cfg[task_name] = sfeat_cfg

    return GraphDatasetMultiTaskConfig(
        graph=g_cfg,
        node_features=nfeat_cfg,
        edge_features=efeat_cfg,
        seed_features_dict=sfeat_task_cfg,
        task_dict=task_cfg_dict,
        feature_groups=feature_groups
    )
