    ...
    def done(self, path : Path) -> OnDiskTaskData:
        ...
        for type, items in self.train_set.items():
            ...
            for item in items:
                ...
                data_path = update_safe_path(train_set_dir, data_path)
        ...
        for type, items in self.validation_set.items():
            ...
            for item in items:
                ...
                data_path = update_safe_path(validation_set_dir, data_path)
                np.save(data_path, item_data, allow_pickle=True)
                item.pop("data")
                item["path"] = str(data_path.relative_to(path))
        ...
        for type, items in self.test_set.items():
            ...
            for item in items:
                ...
                data_path = update_safe_path(test_set_dir, data_path)
                np.save(data_path, item_data, allow_pickle=True)
                item.pop("data")
                item["path"] = str(data_path.relative_to(path))
    ...
    def done(self, path : Path):
        ...
        for edge_item in self.edges:
            ...
            if edge_item["format"] == OnDiskDatasetEdgesFormat.CSV:
                ...
                data_path = update_safe_path(edges_dir, data_path)
        ...
        for feat_item in self.features + self.graph_attributes:
            ...
            data_path = update_safe_path(features_dir, data_path)
...
def update_safe_path(dir_path : Path, data_path : Path):
    # First check if the data_path is already safe.
    # Find the name part by removing the dir_path.
    if data_path.is_relative_to(dir_path):
        data_path_name = str(data_path.relative_to(dir_path))
    # if data_path is too long, use a hash of the path instead
    # data_path is a PosixPath object
    path_len = len(str(data_path))
    if path_len >= 255:
        # Convert the last part of the path to a hash
        data_path_name = Path(hashlib.md5(data_path_name.encode()).hexdigest() + ".npy")
    # replace slash with underscore
    if "/" in str(data_path_name):
        data_path_name = data_path_name.replace("/", "_")
    return Path(dir_path) / data_path_name
